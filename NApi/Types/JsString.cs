...
namespace NApi.Types
{
  public sealed class JsString : JsValue
  {
    ...
    {
      nuint length;
      napi_get_value_string_utf8(Scope.Env.EnvPtr, ValuePtr, null, 0, &length).ThrowIfFailed(Scope);

      sbyte[] buf = new sbyte[length + 1];
      fixed (sbyte* bufStart = &buf[0])
      {
        napi_get_value_string_utf8(Scope.Env.EnvPtr, ValuePtr, bufStart, (nuint)buf.Length, null).ThrowIfFailed(Scope);
